    ...
    while cam.isOpened(): #Making sure the camera is open
        ...
        if result.hand_world_landmarks:#makes sure our code only works when we actually get the data
                ...
                if abs(x1 - x2) <0.015 and abs(y1 - y2) <0.008 :
                    ...
                    elif tx != 0 and lx == 0 and is_touching == False :
                         lx,ly = cx,cy
                    else:
                         cv2.line((frame),pt1=(tx,ty),pt2=(lx,ly),color=(0,255,0),thickness=10)

                        
                elif tx != 0 and lx == 0:#This here is for detecting if we have only pinched 1 time or more than one using the state of tx and lx to see if they are zero or not in this case if tx isnt zero and lx is zero the state is the pinch only has been done 1 time and we need to set is_Touching to False
